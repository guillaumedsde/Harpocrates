...
export default function PopoverMenu(props) {
    ...
    const handleRedact = (event, setId, docId, refresh) => {
        const selection = window.getSelection();
        const selectionRange = getTextSelection(selection.anchorNode.parentNode);
        const section = new SensitiveSection(selectionRange.start, selectionRange.end);
        const date = new Date()
        api.addSensitiveSection(setId, docId, {sensitiveSection: section}).then( () => {
          refresh(date.getMilliseconds(), console.log(section))
        })
      };
      ...
      <ButtonGroup  variant="contained" size="medium" color="primary">
        ...
        : <Button size="medium" onClick={event =>
          handleRedact(
            ...
            props.setName,
            props.docId,
            props.refreshSensitiveSections
          )
        } startIcon={<BorderColor />}>Redact</Button>}
